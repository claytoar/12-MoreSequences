...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points)):
        ...
        circle2.center.attach_to(window)
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4: a ton of black lines on yellow circle'
    window = rg.RoseWindow(400, 300, title)
    circle = rg.Circle(rg.Point(200, 150), 125)
...
def run_test_polygon():
    ...
    # Test 3:
    title = ('Polygon test 3: 4 segments with black lines')
    window = rg.RoseWindow(400, 300, title)
    circle = rg.Circle(rg.Point(200, 150), 100)
...
def run_test_fancy_polygon():
    ...
    # Test 4:
    title = ('Fancy Polygon test 4: 51 red lines on white circle,'
              + ' hops = 25.')
    window = rg.RoseWindow(400, 300, title)
    circle = rg.Circle(rg.Point(200, 150), 100)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(points)):
        ...
        start = points[current_index]
        current_index = (current_index + hops_to_next_point) % len(points)
        stop = points[current_index]
